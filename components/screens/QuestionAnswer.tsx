...
const styles = StyleSheet.create({
  ...
  container: {
    ...
    backgroundColor: '#ffedd5',
    height: 'auto',
    // height: Platform.OS == 'ios' ? height * 0.95 : height * 0.9,
  },
...
const QuestionAnswer = ({route}) => {
  ...
  const updateAnswers = (option: string) => {
    ...
    if (newAnswer[currentLocation] != option) {
      ...
      newAnswer = newAnswer
        .slice(0, currentLocation)
        .concat(newAnswer.slice(currentLocation + 1));
    }
  ...
  const pressFinish = async () => {
    ...
    try {
      ...
      var response = await axios.put(
        `http://3.26.57.153:8000/quiz/put_quiz?quiz_id=${item.quiz_id}`,
        {
          dummy: '',
        },
        {
      ...
      setOverlayMessage(
        'Submission Failed. Please check the network connection',
      );
    } finally {
      setshowOverlay(true);
    }
  ...
  const fetchQuestion = async () => {
    try {
      ...
      var response = await fetch(
        `http://3.26.57.153:8000/quiz/get_quiz?quiz_id=${item.quiz_id}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${userinfo.token}`,
        ...
        <View style={styles.container}>
          <View style={styles.questionNumberContainer}>
            <Text style={styles.questionNumberText}>
              {'Question  ' + (currentLocation + 1)}
            </Text>
          </View>
          <View style={styles.questionTitleContainer}>
            <Text style={styles.questionTitleText}>
              {datas[currentLocation].question}
            </Text>
          </View>
          <View style={{margin: 20}}>
            {datas[currentLocation].options.map((option, index) => (
              <View
                key={index}
                style={
                  answers[currentLocation] == option
                    ? datas[currentLocation].answer.split(' - ')[0] ==
                      answers[currentLocation]
                      ? {...styles.answerRow, backgroundColor: '#68ba7e'}
                      : {...styles.answerRow, backgroundColor: '#c47070'}
                    : styles.answerRow
                }>
                <CheckBox
        ...
        <TouchableOpacity
          style={
            currentLocation != 0
              ...
              : {...styles.previousButton, display: 'none'}
          }
